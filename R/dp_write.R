...
dp_write <- function(data_object, project_path = "."){
  ...
  if(!is_valid_dp_repository(path = project_path))
    stop(glue::glue("Not a dp repsitory. ",
                    "Run `dp_repository_check(\"{project_path}\")` ",
                    "for details"))
  
  if(!"dp" %in% class(data_object))
    stop(cli::format_error(glue::glue("dp_write requires and data_object of ",
  ...
  if(!dir.exists(paths = glue::glue("{project_path}/output_files/RDS_format")))
    dir.create(path = glue::glue("{project_path}/output_files/RDS_format"), 
               recursive = T)
  
  dataobj_path <- glue::glue("{project_path}/",
                             "output_files/RDS_format/data_object.RDS")
  saveRDS(object = data_object, file = dataobj_path)
  data_object <- readRDS(file = dataobj_path)
  
  log_note <- dplognote_get(data_object = data_object, 
  ...
  if(file.exists(glue::glue("{project_path}/.daap/daap_log.yaml"))){
    log_history <- yaml::read_yaml(file = glue::glue("{project_path}/",
                                                     ".daap/daap_log.yaml"))
  ...
  yaml::write_yaml(x = log_history, file = glue::glue("{project_path}/",
                                                      ".daap/daap_log.yaml"))
...
dplognote_get <- function(data_object, project_path){
  ...
  rds_obj_sha1 = digest::sha1(x = make_sha1_compatible(data_object), 
                              environment = F)
  pin_version = get_pin_version(d = data_object, 
                                pin_name = attr(data_object,"dp_name"),
                                pin_description = attr(data_object,
                                                       "branch_description"))
  
  log_note <- c(attrs, rds_file_sha1 = rds_file_sha1, 
