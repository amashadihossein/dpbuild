...
#' @examples  \dontrun{
#' dppkg_modify(project_path = ".", pkg_name = "dpdeploy", 
...
dppkg_modify <- function(project_path = ".",
  ...
  if(length(pkg_version) == 0 | length(pkg_sha) == 0){
    ...
    if(length(pkg_version) != 0)
      warning(cli::format_warning(glue::glue("pkg_version {pkg_version} is ", 
                                             "ignored unless pkg_sha is also ",
                                             "provided")))
    
    if(length(pkg_sha) != 0)
      warning(cli::format_warning(glue::glue("pkg_sha {pkg_sha} is ignored unless ", 
                                             "pkg_version is also provided")))
    
    pkgmeta <- try(get_rspm_pkgmeta(pkg_name = pkg_name))
    
    if("try-error" %in% class(pkgmeta))
      stop(cli::format_error(glue::glue("Failed to retrieve pkgmeta from ", 
  ...
  if(repo_name %in% repo_names_manifest){
    ...
    if(repo_url %in% repo_urls_manifest){
      ...
      if(!repo_nameurl_match){
        if(force_repo_overwrite){
          cli::cli_alert(glue::glue("Overwriting repository url for repo",
                                    " {repo_name} from {repo_url_i} to {repo_url}"))
          renv_manifest$R$Repositories[[reponame_index]]$URL <- repo_url
        }else{
          stop(cli::format_error(glue::glue("URL for repo {repo_name} is ",
    ...
    renv_manifest$R$Repositories <- c(renv_manifest$R$Repositories,
                                      list(list(Name = repo_name, URL = repo_url)))
    if(verbose)
      cli::cli_alert_success(glue::glue("Added repo {repo_name} with url ",
                                        "{repo_url}"))
  }
  ...
  new_pkg_record <- list()
  new_pkg_record[[pkg_name]] <- list(Package = pkg_name, Version = pkg_version,
                                     Source = "Repository", 
  ...
  jsonlite::write_json(x = renv_manifest, path = renv_manifest_path,pretty =T,
                       auto_unbox = T)
