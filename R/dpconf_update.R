...
dpconf_update <- function(project_path = fs::path_wd(),
  ...
  if(!is_valid_dp_repository(path = project_path)){
    ...
    stop(cli::format_error(glue::glue("Not a valid dp repository. Ensure this ",
  ...
  add_gitignore <- setdiff(git_ignore, readLines(file.path(project_path,
                                                           ".gitignore")))
  if(length(add_gitignore) > 0)
    writeLines(glue::glue_collapse({c(git_ignore,add_gitignore)},sep = "\n"),
  ...
  if(length(branch_name) > 0){
    ...
    # Create a branch
    branch_1 <- git2r::branch_create(commit = git2r::last_commit(repo = repo),
                                     name = branch_name)
